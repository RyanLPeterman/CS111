 ...
 * sys_newthread()
 *
 *   Creates a new thread that shares the same address space as the parent process
 *   Thread starts execution at start_function and returns the pid
 *
 *****************************************************************************/
...
sys_newthread(void (*start_function)(void)) {
	...
	asm volatile("int %1\n"
		     : "=a" (pid)
		     : "i" (INT_SYS_NEWTHREAD),
		       "a" (start_function)     // point program counter to start function
		     : "cc", "memory"); 
	return pid;
}
