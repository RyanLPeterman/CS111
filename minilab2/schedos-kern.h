...
typedef struct process {
	...
	// Added Fields
	unsigned int p_run_count;	// holds number of times run
	unsigned int p_share;		// Value to hold the process's share
	unsigned int p_priority;	// Value to hold the process's priority
	unsigned int p_num_tickets; // holds the number of tickets assigned to process
	
	registers_t p_registers;	// Current process state: registers, stack location, EIP, etc. 'registers_t' defined in x86.h
